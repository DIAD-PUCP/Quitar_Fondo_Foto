...
def create_zip_file(images, zip_filename, names):  # Funcion que crea un archivo .zip.
    ...
    try:
        with zipfile.ZipFile(
        ...
        ) as zip_file:  # Abre el archivo .zip en modo escritura
            for i, img in enumerate(images):
                ...
                zip_file.writestr(
                    filename, img_bytes.getvalue()
                )  # Crea y almacena la imagen en el fichero .zip con el nombre y bytes establecidos

        with open(
            zip_filename, "wb"
        ) as f:  # Se crea y abre el archivo .zip final en modo escritura binaria
            f.write(
                zip_buffer.getvalue()
            )  # Ingresa los datos almacenados en el .zip creado anteriormente y lo guarda con el nombre elegido
    except OSError as e:
...
def img_remover(
    ...
    if set == "0":
        ...
        ]  # Selecciona solo los documentos que contengan el formato de imagen aceptado
    ...
    for file in files:  # Se ejecuta archivo por archivo
        ...
        resized_img, new_width, new_height = resize_image(img2, 240, 288)
    ...
    if len(files) == 1:
        st.download_button(
            "Descargar", data=buf, file_name=names[0], mime="image/jpeg"
        )  # Si la imagen solo es una se descarga directamente
        ...
        with open(
            zip_filename, "rb"
        ) as f:  # Abre el archivo .zip en modo lectura binaria
...
def main():
    try:
        ...
        model = st.selectbox(
            "Modelo:", ["u2net", "u2net_human_seg", "birefnet-portrait"]
        )
        ...
        )  # Crea un boton para subir uno o varios archivos
    ...
    if files:
        try:
            img_remover(
                files, "1", None, 300, session
            )  # Abrir los archivos y ejecutar las funcion segun los parametros necesarios
        except ValueError:
